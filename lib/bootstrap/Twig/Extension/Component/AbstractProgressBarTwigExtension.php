...
use WBW\Bundle\BootstrapBundle\Renderer\Component\ProgressBarRenderer;
...
abstract class AbstractProgressBarTwigExtension extends AbstractTwigExtension {
    ...
    protected function bootstrapProgressBar(ProgressBarInterface $progressBar): string {
        ...
        $span = static::h("span", $progressBar->getValue() . "%", ["class" => "sr-only"]);

        $type = ProgressBarRenderer::renderType($progressBar);
        if (null !== $type && 3 !== $this->getVersion()) {
        ...
        $div = static::h("div", $innerHTML, $attributes);

        return static::h("div", $div, ["class" => "progress"]);
    }
}
